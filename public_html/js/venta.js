...
function leer(){
    ...
    $.ajax({
        ...
        if(response==2){
            ...
            +precioUnitario+'</td><td>'+cantidad+'</td><td>'+subtotal+
            '</td><td> <button type="button" onclick="disminuirProductos(this);" class="btn btn-danger"><i class="fa fa-minus"></i></button> <button type="button" onclick="aumentaProducto(this);" class="btn btn-danger"><i class="fa fa-plus"></i></button> <button type="button" onclick="devolucionProducto(this);" class="btn btn-danger"><i class="fa fa-trash"></i></button></td>';
...
function aumentaProducto(id){
    var precioUnitario=parseFloat($(id).parents("tr").find("td")[2].innerHTML);
    var Cantidad=parseFloat($(id).parents("tr").find("td")[3].innerHTML);
    var Subtotal=parseFloat($(id).parents("tr").find("td")[4].innerHTML);
    var datos = {
    ...
    $.ajax({
        ...
        if(response==0){
            alert("Error al agregar un producto mas");
        }else if(response==1){
            var total=parseFloat($('#Total').val());
            Cantidad++;
            Subtotal=Subtotal+precioUnitario;
            $(id).parents("tr").find("td")[4].innerHTML=Subtotal.toFixed(2);
            $(id).parents("tr").find("td")[3].innerHTML=Cantidad.toFixed(2);
            total=(total+precioUnitario).toFixed(2);
            $('#Total').val(total);
        }else if (response==2){
            alert("Ya no hay mas productos disponibles");
        }
    }).fail(function(response){
        alert("Hubo un error en el server, reintentelo de nuevo");
    });
...
function disminuirProductos(id){
    ...
    if (Cantidad>1) {
        ...
        $.ajax({
            ...
            if(response==0){
                alert("Error al quitar un producto");
            }else if(response==1){
                var total=parseFloat($('#Total').val());
                Cantidad--;
                Subtotal=Subtotal-precioUnitario;
